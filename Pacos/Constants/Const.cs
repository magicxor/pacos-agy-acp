...
public static class Const
{
    ...
    public const string SystemPrompt = $"""
                                        ...
                                        14. Если пользователь присылает медиафайл (картинку, видео, аудио, pdf), и в сообщении есть метка "[SYSTEM: Файл из сообщения...: <путь>]", ты ОБЯЗАН использовать инструмент view_file, чтобы просмотреть или послушать его содержимое по указанному пути. НИКОГДА не говори, что ты не можешь смотреть видео или слушать аудио — у тебя есть для этого инструмент view_file.
                                        """;
    ...
    public const string FileDeliveryRuleSystemPrompt = """
                                                    ...
                                                    - Параметры move_file:
                                                      1. sourceDirectory — АБСОЛЮТНЫЙ путь к директории, где сейчас лежит файл. Разрешены ТОЛЬКО две: brain-директория (например /home/agent/.gemini/antigravity-cli/brain) и временная директория из метки "[SYSTEM: Временная директория (не отправляется пользователю): <путь>]". Из любых других директорий перемещение будет отклонено.
    ...
    // Header of the skill index in the steering file; the per-skill lines are generated from
    // AgentSkills.All by ChatWorkspaceProvisioner, so the catalog stays the single source of truth.
    public const string SkillsRuleSystemPromptHeader = """
                                                    # НАВЫКИ (SKILLS)
                                                    - В рабочей директории лежат навыки — подробные инструкции по работе с конкретными инструментами. ОБЯЗАТЕЛЬНО прочитай нужный SKILL.md перед тем, как пользоваться соответствующими инструментами.
                                                    """;
    ...
    public const int MaxTelegramPhotoMaxDimension = 2560;
