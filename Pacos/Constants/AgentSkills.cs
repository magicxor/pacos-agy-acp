    ...
    [
        ...
        new AgentSkill
        {
            ...
            Body = """
                   ...
                   Весь этот раздел — про КОЛИЧЕСТВЕННЫЕ данные, где «темнее = больше»: тонны, люди, проценты, деньги. Для неупорядоченных категорий (языки, религии, партии) цветовая шкала не подходит вообще — см. раздел 4.
                   НЕ задавай backgroundColor внутри объекта dataset вместе с цветовой шкалой: он перебьёт её раскраску и все фичи станут одного цвета. Цвета количественной карты задаются исключительно через объект шкалы options.scales.color.
                   ...
                   Если фичи делятся на несколько НЕУПОРЯДОЧЕННЫХ категорий, готовая палитра D3 из раздела 3 не годится: она даёт плавный градиент, по которому категории не различить, и навязывает порядок и величину там, где их нет («Бразилия в четыре раза больше Аргентины»?). Числовые значения в data при этом становятся кодами категорий, а не величинами.
                   Через цветовую шкалу это в принципе можно сделать: своя функция interpolate с порогами по t даёт чёткие границы цветов, а ticks.callback подменяет числа на названия категорий. Способ рабочий, но в нём две функции, которые должны согласовываться между собой и с min/max шкалы, а пороги по t считаются вручную ((код - min) / (max - min)): промахнулся на шаг — и цвета разъехались с подписями молча, без всякой ошибки.
                   Поэтому по умолчанию делай проще и без единой функции — ОДИН ДАТАСЕТ НА КАТЕГОРИЮ:
                   - у каждого датасета свой label (название категории: "Испанский"), свой сплошной backgroundColor и свой список фич в data;
                   - строки данных здесь без value: { "feature": "Brazil" }. Значение не нужно — раскраска задана цветом датасета, а лишний value ещё и вылезет цифрой под названием страны, если включить подписи (раздел 6);
                   - легенда получается сама, названиями категорий: options.plugins.legend = { "display": true, "position": "right" };
                   - цветовую шкалу выключай, но axis у неё оставляй: scales.color = { "axis": "x", "display": false } — без axis шкала выкинет ошибку даже скрытая.
                   ПОРЯДОК ДАТАСЕТОВ ВАЖЕН: chart.js рисует их с конца, поэтому серую подложку (showOutline: true + outlineBackgroundColor) задавай в ПОСЛЕДНЕМ датасете, а во всех остальных ставь showOutline: false. Если подложку задать в первом, она накроет собой все остальные категории и они просто исчезнут с карты.
                   Когда категорий много, а легенда не нужна (например, подписи прямо на фичах), есть второй способ: один датасет, а в backgroundColor — МАССИВ цветов той же длины и в том же порядке, что data. Легенды при нём не будет.
                   Цвета бери осмысленные: цвета флагов для языков, партийные цвета для выборов, зелёный/жёлтый/серый для климата. Больше 6-7 категорий читается плохо — объединяй мелкие в «прочие».
