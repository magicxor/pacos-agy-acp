    ...
    [
        ...
        new AgentSkill
        {
            ...
            Body = """
                   ...
                   Этот раздел — про КОЛИЧЕСТВЕННЫЕ данные, где «темнее = больше»: тонны, люди, проценты, деньги. Неупорядоченные категории (языки, религии, партии) — раздел 4.
                   ...
                   ## 4. Категориальная раскраска (языки, религии, партии, климат)
                   Неупорядоченные категории раскрашивай ОДНИМ ДАТАСЕТОМ НА КАТЕГОРИЮ, а не цветовой шкалой из раздела 3:
                   - у датасета свой label (название категории: "Испанский"), свой сплошной backgroundColor и свой список фич в data;
                   - строки данных без value: { "feature": "Brazil" }. Цвет задан датасетом, а лишний value вылезет цифрой под названием страны, если включить подписи (раздел 6);
                   ...
                   На количественной карте обычную легенду датасета убирай: options.plugins.legend.display = false — цветовая шкала её заменяет. На категориальной карте (раздел 4) она, наоборот, нужна — это и есть ключ к цветам.

                   ## 7. Приближение к региону (кадрирование)
                   ...
                   Проверенный рабочий конфиг — бери его за основу и меняй только label, title и data. Передавай строгим JSON (двойные кавычки), проекцию не добавляй. Стран много, поэтому подписи здесь выключены (см. раздел 6).
                   ```
                   {
                     "type": "choropleth",
                     "data": {
                   ...
                   ## Пример категориальной карты (раздел 4)
                   Официальные языки Южной Америки: датасет на язык, легенда с названиями языков, цветовая шкала выключена, подложка — в последнем датасете. Ни одного value и ни одной функции.
                   ```
                   {
                     "type": "choropleth",
                     "data": {
                       "datasets": [
                         {
                           ...
                           "showOutline": false,
                           "data": [
                             { "feature": "Argentina" }, { "feature": "Bolivia" }, { "feature": "Chile" },
                             { "feature": "Colombia" }, { "feature": "Ecuador" }, { "feature": "Paraguay" },
                             { "feature": "Peru" }, { "feature": "Uruguay" }, { "feature": "Venezuela" }
                           ]
                         },
                         {
                           "label": "Португальский",
