    ...
    [
        ...
        new AgentSkill
        {
            ...
            Body = """
                   ...
                   - ПЕРЕД покраской ВСЕГДА вызывай list_maps с mapName и бери имена фич оттуда: в data нужны точные алиасы этой карты (регистр не важен), незнакомое имя — ошибка 400. Не угадывай названия по памяти, особенно у регионов страны.
                   ...
                   - Когда карта посвящена ОДНОЙ стране («безработица во Франции», «население Германии по землям»), строй её на карте самой этой страны (map: ISO-3), а не на мировой карте, обрезанной по границам этой страны. На мировой карте страна — одна фича, то есть сплошное пятно одного цвета: её регионы не покрасить и не подписать, а в кадр лезут куски соседей. Мировую карту бери, только если пользователь прямо просил показать страну на фоне мира или в окружении соседей. Как навести на такую карту кадр — раздел 7.
                   ...
                   - строки данных без value: { "feature": "Brazil", "label": "Бразилия" } — label нужен для подписи (раздел 6), а лишний value вылезет цифрой под названием страны, ведь цвет и так задан датасетом;
                   ...
                   На категориальной карте (раздел 4) значение в data не кладут, поэтому название категории для подписи бери из датасета: "formatter": "function(v, ctx) { return [v.label, ctx.dataset.label]; }" — на стране встанут её название и, например, её язык.
